...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        i = 1
        while i < T:
            alpha[i] = (B[:, x[i]] * (A.T @ alpha[i - 1].T)).T
            i = i + 1
    ...
    def generate_sentence(self, sentence_length):
        ...
        z_state = np.arange(A.shape[0])
        z_seq = np.zeros(sentence_length).astype(int)
        z_seq[0] = np.random.choice(z_state, p=pi.flatten())

        x_state = np.arange(B.shape[1])
        x_seq = np.zeros(sentence_length).astype(int)
        x_seq[0] = np.random.choice(x_state, p=B[z_seq[0]])

        i = 1
        while i <= sentence_length - 1:
            z_seq[i] = np.random.choice(z_state, p=A[z_seq[i - 1]])
            x_seq[i] = np.random.choice(x_state, p=B[z_seq[i]])
