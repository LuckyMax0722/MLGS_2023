...
class NeuralTPP(torch.nn.Module):
    ...
    def get_log_density(
        ...
        #######################################################
        log_probs = distribution.log_prob(times)
        log_probs_masked = log_probs.masked_fill(~mask, torch.zeros(1))
        log_density = log_probs_masked.sum(dim=1)
        #######################################################
        return log_density
    ...
    def get_log_survival_prob(
        ...
        #######################################################
        log_cdf = distribution.log_cdf(times)
        log_surv = 1 - log_cdf
        log_surv_masked = log_surv.masked_fill(~mask, 0.0)
        log_surv_last = log_surv_masked[:, -1]
        #######################################################
    ...
    def embed_history(
        ...
        output, _ = self.embedding_rnn(encoded_times)

        batch, max_seq_length, _ = encoded_times.shape

        emb_dim = self.hidden_dim

        history_emb = torch.zeros(batch, max_seq_length, emb_dim)
