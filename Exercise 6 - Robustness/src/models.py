...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int,
        ...
        count_top_class = class_counts[top_class].item()
        p_A_lower_bound = lower_confidence_bound(count_top_class, num_samples, alpha)
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float,
        ...
        ##########################################################
        # YOUR CODE HERE
        count_top_class = class_counts.max().item()
        count_other_classes = class_counts.sum().item() - count_top_class
        p_value = binom_test(count_top_class, count_top_class + count_other_classes, p=0.5)
        if p_value > alpha:
            return SmoothClassifier.ABSTAIN
        else:
            return torch.argmax(class_counts).item()
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int,
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                ##########################################################
                # YOUR CODE HERE
                perturbed_images = (batch + random_numbers * self.sigma).clamp(0, 1)
                predictions = self.base_classifier(perturbed_images)
                _, predicted_classes = torch.max(predictions, dim=1)
                class_counts += torch.bincount(predicted_classes, minlength=self.num_classes)
