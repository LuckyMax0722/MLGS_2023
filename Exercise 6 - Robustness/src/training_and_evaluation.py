...
def training_step(x: torch.Tensor, y: torch.Tensor, model: nn.Module,
    ...
    loss, logits = loss_function(model, x, y, **loss_args)

    loss.backward()
    optimizer.step()

    _, predicted_labels = logits.max(dim=1)
    accuracy = (predicted_labels == y.squeeze()).sum().item() / y.size(0)
...
def standard_loss(model: nn.Module, x: torch.Tensor, y: torch.Tensor):
    ...
    # YOUR CODE HERE
    logits = model(x)
    loss = cross_entropy(logits, y.squeeze())
    ##########################################################
    return loss, logits
...
def prediction(x: torch.Tensor, y: torch.Tensor, model: nn.Module,
    ...
    if attack_function is not None:
        perturbed_batch = attack_function(model, x, y, **attack_args)
...
def loss_function_adversarial_training(model: nn.Module, x: torch.Tensor, y: torch.Tensor, **attack_args):
    ...
    loss_pert = cross_entropy(logits_pert, y.squeeze()).mean()
    ##########################################################
...
def evaluate_robustness_smoothing(smoothed_classifier: nn.Module,
    ...
    for x, y in test_loader:
        ...
        noise = torch.randn((num_samples_1,) + x.shape) * sigma
        perturbed_inputs = x + noise

        # Predict most likely class
        logits = smoothed_classifier(perturbed_inputs)
        predicted_class = torch.argmax(logits.sum(dim=0))

        # Certification using additional noise samples
        noise = torch.randn((num_samples_2,) + x.shape) * sigma
        perturbed_inputs = x + noise
        logits = smoothed_classifier(perturbed_inputs)
        predicted_probabilities = nn.functional.softmax(logits, dim=1)
        max_probabilities, _ = predicted_probabilities.max(dim=1)

        # Check if the prediction can be certified
        certified = max_probabilities.ge(1 - alpha).any().item()

        if not certified:
            abstains += 1
        elif predicted_class != y.item():
            false_predictions += 1
        else:
            correct_certified += 1
            radii.append(sigma)
